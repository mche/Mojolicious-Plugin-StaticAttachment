package Mojolicious::Plugin::AttachmentStatic;
...
sub parse_paths {
  ...
  my $paths = $self->paths;
  while (my ($path, $val) = splice(@$args,0,2)) {
    ...
    $val->{filename} = quote $val->{filename};
    utf8::encode($val->{filename})
      if utf8::is_utf8($val->{filename});
...
our $VERSION=0.001; # End of Mojolicious::Plugin::AttachmentStatic
...
=head1 Mojolicious::Plugin::AttachmentStatic

¡ ¡ ¡ ALL GLORY TO GLORIA ! ! !

=head1 NAME

Mojolicious::Plugin::AttachmentStatic - Add 'Content-Disposition' header for specified statics.
...
=head1 SYNOPSIS
  ...
  $app->plugin('AttachmentStatic' => paths => ['/foo.txt', 'bar.portable.doc.format'=>{content_type=>'application/pdf', filename=>"файлик.pdf"}])
